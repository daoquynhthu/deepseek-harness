...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-markdown`](../packages/memory/memory-markdown) | [`tool-memory`](../packages/memory/tool-memory) | - | The provider persists curated knowledge as indexed markdown under the harness home and serves keyword recall; tool-memory is the model-facing consumer with memory_search/get/set plus session-start injection. |
