...
| `@deepseek-ai/dsh-tool-memory` | `memory_get`, `memory_search`, `memory_set` | `ctx.tools`, `ctx.memory`, `ctx.systemPrompt`, `a calling Agent for the session-bound tools` | `tool/call`, `tool/result`, `memory files + the SQLite chunk index (memory_set)`, `a "Project memory" user message (session-start injection)` | - | memory_search, memory_get, and memory_set are the model-facing consumer of the curated-memory seam; memory_set additionally reindexes the written file. The same package injects up to `maxInjectedChunks` evergreen chunks into each session start as a "Project memory" snapshot. |
...
Read one memory file completely, for example MEMORY.md or a specific session archive.
...
Source: [`packages/memory/tool-memory/src/operations.ts`](../packages/memory/tool-memory/src/operations.ts)

### `memory_search`

Search curated cross-session memory and return the strongest matching knowledge chunks.
...
Write one memory file with full markdown content, replacing any existing content.
