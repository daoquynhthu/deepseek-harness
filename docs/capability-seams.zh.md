...
flowchart LR
  ...
  pkg_memory["memory"]
  svc_memory["ctx.memory<br/>Curated-memory seam"]
  ...
  pkg_memory_markdown --> svc_memory
...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-markdown`](../packages/memory/memory-markdown) | [`tool-memory`](../packages/memory/tool-memory) | - | 提供方将策展的知识持久化为宿主主目录下带索引的 markdown，并提供关键词召回；tool-memory 是面向模型的消费方，提供 memory_search/get/set 及会话开始注入。 |
